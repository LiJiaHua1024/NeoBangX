# NeoBangX

NeoBangX —— 面向中学英语教师的 AI 辅助教学平台。

> 版本：v1.2
> 核心变更：引入使用码系统、管理后台、后端 SQLite 数据存储与智能错题迁移。

---

## 1. 功能概览

- **33 个中学英语教学 AI 工具**：新增独家功能“智能错题迁移”，覆盖语篇分析、教学设计、命题辅助、作文批改、试卷讲评等。
- **翻译（通用工具）**：原文与译文**左右分栏对照**（窄屏自动改成上下分屏），一侧滚动按进度带动另一侧对齐（强制开启，两端位置同步）；原文栏可折叠，折叠时译文居中。**窄屏上译文栏一开始不占地方**（原文栏独占几乎整屏），按下「翻译」才按同一条非线性动画放出来；打字时键盘弹起，译文栏还会自动收成一行把高度让给原文栏——因为这个工具的输入区在上半屏，跟着键盘一起往上收只会压掉输入空间。上传按钮宽屏在原文栏右侧、窄屏在底栏。原文栏可直接粘贴，也可以上传 Word / PDF / TXT，或拍照识别印刷试卷与手写作文（与「自由对话」同一套上传与识别流程）。语言对给出中/英/日/韩/法/德/西/俄/意/葡/阿/泰/越等清单，原文默认自动检测、译文默认简体中文，原文选成简体中文时译文自动换成英语（美式）。**不是实时翻译**：点「翻译」才开始，流式出字可停止。同一份材料改了原文再翻，结果**写回同一条历史记录**（按钮此时显示「更新翻译」），不在列表里刷出一串半成品；想另起一条就点译文栏的「新的翻译」。思考过程只在思考期间出现，正文一出即整条隐藏。
- **图片识别（识别图片文字）**：在「通用工具」里拍照或上传试卷、手写作文，转成可编辑文字。转录规则由所在工具决定，不需要用户选：作文批改收到的学生手写稿按涂改后的最终状态逐字转录（保留拼写与语法错误），其余工具一律只转录印刷原文（学生答案、勾画、批注自动忽略）；只有「识别图片文字」「自由对话」「翻译」三处两种素材都可能出现，才在界面上给选择。一次可拍多张（整卷多页），其他工具的「上传」入口也能直接走识别，识别结果落进输入框继续用。**点图放大即是编辑器**：可转正、可裁掉桌面与边缘，裁剪退出裁剪模式就见到结果（实时预览，不改动原图也不重新编码），电脑上拖框与滚轮缩放、手机上双指缩放拖把手，满意了点「完成」写入这张图片，第一次动图前留底、随时「还原原图」。
- **使用码系统**：使用码即登录凭据，无需注册；无码不可用，按次计费，用完锁定。
- **管理后台**：内网访问，生成/管理使用码、查看使用日志、配置 LLM API。
- **流式生成**：SSE 打字机效果，生成过程可中止；中断后可「继续生成」接着往下写，对结果不满意可「重新生成」整篇重来——旧结果不丢，底部状态条的版本切换器能在新旧几版之间来回对比。
- **历史 / 收藏**：存于浏览器 localStorage（后端不存用户数据）。
- **双线路支持**：同一套应用可由多条线路（主线路 + 备份线路）提供服务，历史、收藏与主题 / 模型偏好都在两条线路间自动镜像；也可导出 / 导入 JSON 手动搬运。见 [9. 数据存储](#9-数据存储)。
- **多主题切换**：毛玻璃 / 科技感视觉风格。

---

## 2. 技术栈

| 层 | 选型 |
| --- | --- |
| 后端框架 | FastAPI |
| Python | 3.13 |
| 包管理 | UV |
| LLM 统一层 | LiteLLM |
| 数据库 | SQLite |
| 前端 JS | Alpine.js |
| 前端 CSS | Tailwind CSS v4（预编译静态 CSS，浏览器无编译开销） |
| 容器 | Docker + Supervisor |

---

## 3. 项目目录结构

```
/NeoBangX
├── backend/
│   ├── app/
│   │   ├── main.py              # 主站服务入口（:8000）
│   │   ├── admin_main.py        # 管理后台服务入口（:8001）
│   │   ├── config.py            # pydantic-settings 配置
│   │   ├── database.py          # SQLite 连接
│   │   ├── models.py            # SQLAlchemy 数据模型
│   │   ├── deps.py              # 认证依赖
│   │   ├── routers/
│   │   │   ├── tools.py         # 工具元数据 API
│   │   │   ├── chat.py          # 流式调用 + 标题生成
│   │   │   ├── auth.py          # 使用码激活 / 状态
│   │   │   └── admin.py         # 管理后台 API
│   │   └── services/
│   │       ├── llm.py           # LiteLLM 调用
│   │       ├── migration.py     # 智能错题迁移规则与错因解析
│   │       ├── prompt_loader.py # Prompt 加载
│   │       ├── usage_code.py    # 使用码生成/校验/扣次
│   │       └── runtime_config.py# 运行时配置管理
│   ├── data/                    # SQLite 数据目录（运行时创建）
│   ├── mock_backend/            # 测试用假后端（不进镜像，见 4.6）
│   ├── tests/
│   ├── pyproject.toml
│   └── .env.example
├── frontend/                    # 主站前端（Alpine.js + Tailwind v4，纯静态）
├── admin-frontend/              # 管理后台前端
├── prompts/                     # 33 个工具 Prompt 文件 + 共用的续写指令
├── Dockerfile
├── docker-compose.yml
├── supervisord.conf             # Docker 内双进程管理
├── README.md
└── docs/
    └── API_CONTRACT.md           # 前后端 API 契约
```

---

## 4. 本地开发

### 4.1 环境准备

- 安装 Python 3.13
- 安装 UV：[https://docs.astral.sh/uv/](https://docs.astral.sh/uv/)
- 准备 LLM API Key（推荐 OpenRouter：[https://openrouter.ai/keys](https://openrouter.ai/keys)）

### 4.2 配置环境变量

```bash
cd backend
cp .env.example .env
```

编辑 `backend/.env`，填入 `OPENROUTER_API_KEY` 或 `LLM_API_KEY` 等。

> 该文件为**唯一**配置入口，本地开发与 Docker 部署共用同一份（见 [5. Docker 部署](#5-docker-部署)）。

### 4.3 安装依赖并启动

```bash
cd backend
uv sync

# 启动主站（端口 8000）
uv run uvicorn app.main:app --host 0.0.0.0 --port 8000 --reload

# 另开一个终端启动管理后台（端口 8001）
uv run uvicorn app.admin_main:app --host 0.0.0.0 --port 8001 --reload
```

### 4.4 访问服务

- 主站：http://localhost:8000/
- 主站 API 文档：http://localhost:8000/docs
- 管理后台：http://localhost:8001/
- 管理后台 API 文档：http://localhost:8001/docs

### 4.5 初始使用码

首次启动时，如果数据库中没有任何使用码，后端会自动创建一把 **无限额度的使用码（NBXU-...）**，并写入数据目录下的 `bootstrap_code.txt`（不打印到日志）。请取出并妥善保存。

管理后台本身零登录（仅内网访问），不需要使用码即可进入并生成更多使用码。旧版本遗留的 `bootstrap_admin.txt` 已失效，可手动删除；其中的 NBXA 码仍可作为无限额度使用码继续使用。

### 4.6 用假后端测前端（不烧钱）

改前端要在浏览器里验证时，不必启动真实后端、也不必配 API Key。假后端会托管前端并打桩全部主站 API，LLM 输出由本地场景引擎按需演出——包括"生成到一半中断"这种真实链路里很难构造的情况。

```bash
cd backend
uv run python -m mock_backend.main          # http://127.0.0.1:8000/ 就是完整可点的前端
```

在输入框首行写一行指令即可指定这次请求演什么，例如：

```text
#mock mid-error at=30     # 出 30 个 token 后报错 → 错误卡
#mock mid-truncate at=30  # 直接断流 → 「生成中断，内容可能不完整」
#mock cancel              # 慢速流，留出点「停止」的窗口
#mock paper               # 合法试卷样本，用于测解卷/打印/导出
```

控制台（切场景、看前端实际发出的请求、改额度）在 `http://127.0.0.1:8000/__mock__/`。它还能充当 OpenAI 兼容的假 LLM 上游，把真实后端的 provider 指过来，零成本测多通道 fallback 与计费链路。

- 给人看的完整说明：[backend/mock_backend/README.md](backend/mock_backend/README.md)（场景清单、控制台、镜像双线路、浏览器验收清单、已知差异）
- **给自动化测试的 Agent 抄的操作闭环**：[docs/MOCK_BACKEND_TESTING.md](docs/MOCK_BACKEND_TESTING.md)（起服务 → 设场景 → 驱动页面 → 断言 → 复位，含常用配方）

---

## 5. Docker 部署

### 5.1 构建并运行

配置统一使用 `backend/.env`（与本地开发共用）：

```bash
# 1. 首次部署：生成配置文件并填入 API Key
cd backend
cp .env.example .env
# 编辑 .env，填入 LLM_API_KEY / CHORES_API_KEY 等

# 2. 构建并启动（在项目根目录执行）
cd ..
docker-compose up --build -d
```

### 5.2 更新部署

```bash
git pull
docker-compose up --build -d
```

> **升级提示**：v1.9.0 起使用码不再区分类型，首次启动会自动删除数据库中的 `code_type` 列（不可逆），升级前建议备份数据卷中的 `neobangx.db`。

> 配置由 docker-compose 的 `env_file` 在**运行时**注入容器（`.env` 不会被打进镜像层，镜像可安全分发）。修改 `.env` 后执行 `docker-compose up --build -d` 重建生效。管理后台中修改的配置（模型、API Key 等）存储在 SQLite 数据卷中，重建容器不会丢失。
>
> ⚠️ **首次部署务必修改 `JWT_SECRET`**（默认值公开写在源码里，可被用来伪造登录票据；未修改时管理后台会持续显示安全警告）。最简单的方式是在管理后台安全警告横幅上点「**一键生成新密钥**」——密钥自动写入数据卷并在重启后生效；也可手动生成：`python -c "import secrets; print(secrets.token_urlsafe(48))"` 后填入 `.env`（环境配置优先于数据卷文件）。

### 5.3 端口说明

| 端口 | 用途 | 公网暴露 |
|------|------|----------|
| 8000 | 主站服务 + 前端 | 通过 FRP/Nginx/Cloudflare Tunnel 暴露 |
| 8001 | 管理后台 | 局域网内直接访问（零鉴权，仅限可信内网，勿映射公网）|

### 5.4 访问服务

- 主站：http://localhost:8000/
- 管理后台：从局域网其他电脑访问 `http://<服务器内网IP>:8001/`（本机调试用 localhost）

### 5.5 停止服务

```bash
docker-compose down
```

---

## 6. 配置项说明

配置统一由 `backend/.env` 提供（本地开发与 Docker 部署共用）。读取优先级：环境变量 > `.env` 文件 > 默认值；Docker 部署时由 docker-compose 的 `env_file` 在运行时注入为环境变量，`.env` 不进入镜像层。

> ⚠️ 路径类配置（`PROMPTS_DIR` 等）的相对路径只对本地开发有意义。Docker 容器内的路径已由镜像 ENV 与 compose `environment` 钉死为 `/app/...` 绝对路径，`.env` 中的相对路径在容器内会被忽略，请勿参照修改。

| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `APP_HOST` | `0.0.0.0` | 主站监听地址 |
| `APP_PORT` | `8000` | 主站监听端口 |
| `ADMIN_HOST` | `0.0.0.0` | 管理后台监听地址 |
| `ADMIN_PORT` | `8001` | 管理后台监听端口 |
| `PROMPTS_DIR` | `../prompts` | Prompt 文件目录（仅本地开发；容器内固定 `/app/prompts`）|
| `STATIC_DIR` | `../frontend` | 主站静态文件目录（仅本地开发；容器内固定 `/app/frontend`）|
| `ADMIN_STATIC_DIR` | `../admin-frontend` | 管理后台静态文件目录（仅本地开发；容器内固定 `/app/admin-frontend`）|
| `DATA_DIR` | `./data` | SQLite 数据目录（容器内固定挂载卷 `/app/data`）|
| `JWT_SECRET` | `neobangx-dev-secret-change-me` | JWT 密钥（生产务必修改） |
| `JWT_EXPIRE_DAYS` | `365` | JWT 有效期 |
| `OPENROUTER_API_KEY` | 空 | 兼容旧版的 OpenRouter API Key |
| `DEFAULT_MODEL` | `openrouter/google/gemini-2.0-flash` | 默认模型 |
| `MODELS` | `openrouter/google/gemini-2.0-flash` | 可用模型列表，逗号分隔 |
| `LLM_API_KEY` | 空 | 主 AI API Key（优先级高于 OPENROUTER_API_KEY） |
| `LLM_BASE_URL` | 空 | 主 AI Base URL |
| `LLM_MODEL` | 空 | 主 AI 模型（留空则使用 DEFAULT_MODEL） |
| `CHORES_API_KEY` | 空 | Chores AI API Key（标题生成等轻量任务） |
| `CHORES_BASE_URL` | 空 | Chores AI Base URL |
| `CHORES_MODEL` | 空 | Chores AI 模型（留空则回退主 AI） |
| `OCR_MODEL` | 空 | 图片识别所用视觉模型（**必须支持图片输入**；留空则跟随默认模型） |
| `OCR_MAX_TOKENS` | `8192` | 图片识别单次输出上限（整卷转录比普通工具长得多） |
| `MAX_TOKENS` | `4096` | 最大输出 token 数 |
| `TIMEOUT` | `120` | LLM 调用超时时间（秒） |
| `SSE_RETRY_TIMEOUT` | `30000` | SSE 客户端重连时间（毫秒） |
| `LOG_PAYLOAD` | `false` | 是否记录每次请求的原始输入 / 渲染 Prompt / 输出（元数据始终记录） |
| `LOG_RETENTION_DAYS` | `0` | 使用日志保留天数，超期每日自动清理；`0` 表示永久保留 |

**模型的用途与禁用**（在管理后台「模型管理」里配置，不写在 `.env`）：

每个模型有三个用途能力位，另有一个上一级开关：

| 维度 | 取值 | 作用 |
|------|------|------|
| 启用模型 | 开 / 关 | 上一级开关；关闭后该模型在用户端、OCR、Chores 三处**全部**不可用，下方用途不再生效 |
| 用途：用户可用 | 勾 / 不勾 | 勾上才出现在 8000 用户端的模型下拉里 |
| 用途：用于 OCR | 勾 / 不勾 | 可作为「识别图片文字」的识别模型（须支持图片输入） |
| 用途：用于 Chores | 勾 / 不勾 | 可作为标题生成等轻量任务的模型 |

未禁用的模型**至少要勾一项用途**，否则它没有任何入口能到，保存时会被拦下。
历史数据只有旧的「仅 Chores」单开关时，读取时自动迁移：`user_usable = !chores_only`、
`chores_usable = true`、`ocr_usable = false`；老数据完全没有用途字段时，按
**用户可用 + 用于 Chores** 回填（不默认开放 OCR）。迁移只在读取时发生，故管理后台
把模型列表保存一次后，库里就是新的三能力位结构。

`LLM_BASE_URL` / `LLM_API_KEY` / `CHORES_BASE_URL` / `CHORES_API_KEY` 这几个旧键
**只能由环境变量提供**（管理后台里没有对应输入框）。首次启动若 `llm_providers` 表为空，
会按它们生成首个 Provider 并绑定全部模型；已存在但地址为空的自动迁移 Provider 也会被补齐。

**修改 Tailwind 工具类后重新构建**：在项目根目录执行 `npm ci`，再执行 `npm run build:css`。
两端的 HTML 和 JS 都纳入扫描，包含 Alpine 条件分支与动态模板中的完整类名；不要拼接 Tailwind
类名片段，新增动态工具类时写出完整名称。两个 `tailwind.css` 产物应随源码一起保存，部署仍是纯静态文件，
服务器和 Docker 镜像无需安装 Node。仅改自定义 `styles.css` 无需构建。版本由 `package-lock.json` 锁定，
不会因 CDN 更新而改变样式。采用 [Tailwind 官方的静态扫描方式](https://tailwindcss.com/docs/detecting-classes-in-source-files)。

**静态资源不用手改版本串，也别加回去**：`frontend/` 与 `admin-frontend/` 的文件名不带
内容哈希，而 `index.html` 里的本地资源（JS / CSS / 图片）一律**不带** `?v=` 查询串。带 query 的资源
会被缓存中间件当成「版本化资源」下发 immutable 长缓存，于是「改了文件但忘了改版本串」时浏览器会一直
跑旧代码，升级后出现「新 HTML + 旧 JS/CSS」的错配（管理端曾因此让三个用途勾选框全部显示为未勾选，
翻译工具上线时又让工作区直接变成空白，且手机端无法强退出缓存刷新）。不带版本串走 `no-cache` + `etag`：
文件变了下一次加载自动就是新的，没变则 304，几乎零成本，也不需要任何人工纪律。
两处入口页（`index.html` / `bridge.html`）引用同名模块时必须用同一个 URL——镜像桥接页与主站若各缓存
一份，协议版本不一致会让线路镜像**静默失效**（`backend/tests/test_static_cache.py` 两条测试守着这两点）。

---

## 7. 使用码系统

### 7.1 使用码

所有使用码统一为普通使用码（`NBXU` 前缀），额度支持有限次数或无限（`-1`），不再区分管理员/用户类型。

### 7.2 使用码格式

示例：

```
NBXU-3XXX-XXXX-XXXX
```

### 7.3 计量规则

- 每次工具生成扣减 1 次额度。
- 无限额度码（额度 `-1`）不扣减次数，但仍记录使用日志。
- 额度用完后直接锁定，无法继续使用。
- 管理后台可把某个使用码的已用次数重置为 0（剩余额度随之恢复，使用日志保留）。
- 管理后台可把使用码额度改为无限（已用清零），也可从无限改回有限（已用从 0 开始计数）。

### 7.4 使用流程

1. 管理后台生成使用码。
2. 用户在主站首页或左下角输入使用码并验证。
3. 使用码信息保存于浏览器，后续自动携带。
4. 额度用完时提示重新获取使用码。

---

## 8. API 契约

完整接口文档见 `docs/API_CONTRACT.md`。

核心接口：

- `POST /api/auth/activate` — 验证使用码，返回 JWT
- `GET /api/auth/me` — 当前使用码状态
- `GET /api/tools/` — 工具元数据 + 模型列表
- `POST /api/chat/migration/analyze` — 非流式错因分析（不扣费）
- `POST /api/chat/migration/quota` — 最终生成额度预检查（不扣费）
- `POST /api/chat/stream` — 流式调用（需 Authorization；`continue_from` 传已生成正文即为续写）
- `POST /api/chat/stop` — 中止生成
- `POST /api/chat/title-jobs` — 幂等创建持久标题任务
- `POST /api/chat/title-jobs/status` — 批量对账标题任务
- `POST /api/chat/title` — 旧客户端同步标题生成（兼容）
- `GET /api/admin/stats` — 管理后台统计
- `GET /api/admin/codes` — 使用码列表
- `POST /api/admin/codes` — 生成使用码
- `PATCH /api/admin/codes/{id}` — 启用/禁用/修改额度
- `POST /api/admin/codes/{id}/reset-usage` — 重置已用次数
- `GET /api/admin/logs` — 使用日志列表（支持使用码 / 工具 / 模型 / 状态 / 时间范围筛选）
- `GET /api/admin/logs/summary` — 日志聚合统计（请求数、成功/停止/异常、总 Tokens、平均耗时）
- `GET /api/admin/logs/{id}` — 单条日志详情（含原始输入 / 渲染 Prompt / 输出，受记录开关控制）
- `POST /api/admin/logs/purge` — 手动清理过期日志
- `GET /api/admin/config` — 查看配置
- `PUT /api/admin/config` — 更新配置（含日志记录开关与保留天数、线路镜像开关与线路地址）

---

## 9. 数据存储

| 数据 | 存储位置 |
|------|----------|
| 使用码信息 | 后端 SQLite |
| 使用日志 | 后端 SQLite（元数据 `usage_logs`；原始输入/输出 `log_payloads`，按开关记录） |
| 标题任务 | 后端 SQLite（`title_jobs`）。为让标题生成不依赖网页存活，任务会暂存输入/输出摘要；进入成功或最终失败状态时立即清空正文，只保留状态、结果与错误元数据 7 天。业务历史本身仍不存后端。 |
| LLM / API 配置 | 后端 SQLite（管理后台维护） |
| 历史记录 | 浏览器 localStorage（列表索引 + 每条正文分开存，超出配额时自动淘汰最旧记录）。一条记录默认只存一份结果；「重新生成」过的记录会在正文里多带一个版本数组（`versions` + `activeVersionId`，最多 8 版、总字数封顶），旧结果因此不会丢，底部状态条的切换器用来回看。未重新生成过的记录没有这个字段，与旧数据形状完全一致。 |
| 标题补交日志 | 浏览器 localStorage（`nbx_title_jobs`）。正文历史先落盘，再异步提交标题任务；若关页时 POST 尚未送达，下次打开按同一 job ID 幂等补交。只保存在当前 origin，不随导入或线路镜像传播，终态后删除。 |
| 收藏 | 浏览器 localStorage |
| 主题 / 选中模型 | 浏览器 localStorage（`nbx_theme` / `nbx_model` 为当前值，`nbx_prefs` 存镜像同步用的带时间戳副本） |

除标题任务运行所需的临时摘要与 7 天状态元数据外，后端不持久化聊天历史、收藏等业务数据；历史与收藏仍只存在用户浏览器里。标题进入终态后会立即清空任务里的输入/输出正文。
上面这张表里唯一「多出来」的两行，是为了让同一套应用的两条线路（主线路与备份线路）
能共用同一份历史、收藏与偏好设置：

| 能力 | 说明 |
|------|------|
| 导出 / 导入 | 右栏底部两个按钮，把历史、收藏与主题 / 模型偏好导出成一份 JSON，可再次导入并与本机数据合并（条目按 `id` 取新，偏好按时间戳取新）。用于备份、换设备、以及镜像失效时手动搬运。文件由用户自己保管，不经过后端。 |
| 线路镜像 | 多条线路（主线路 + 备份线路）互嵌一个隐藏的同站 iframe，把本机变更推进对方 origin 自己的 localStorage。**方向是「推」不是「拉」**：备份线路要顶的正是主线路挂掉的那一刻，那时主线路的页面加载不了、也就读不到它的存储，提前推进去故障时才有得用。无后端参与数据。历史、收藏、主题与选中模型一并镜像（同一套 LWW 判据），对端有、本线路没有的主题 / 模型不套用。 |

### 线路镜像的配置

默认**关闭**（单线路部署开启它没有任何作用）。开关与线路地址在
**管理后台 →「线路镜像」**里设置，也可用环境变量 `MIRROR_ENABLED` / `MIRROR_ORIGINS`
（管理后台的保存会覆盖环境变量的值）：

```ini
MIRROR_ENABLED=true
# 逗号分隔，必须是纯 origin（含协议与端口，不带路径）
MIRROR_ORIGINS=https://www.example.com,https://cf.example.com
# 内网 / IP 测试同样可用
# MIRROR_ORIGINS=http://192.168.1.10:8000,http://192.168.1.10:8001
```

地址一律填**完整 origin**：前端用它拼 `{origin}/static/bridge.html`，并与
`location.origin` / `event.origin` 做全等比较，所以带路径、漏端口都会让比对失败。
两条线路是对等的（不分主备，顺序随意）。代码里没有任何硬编码域名——开源部署
各有各的域名，内网测试还可能是 IP + 端口。配置从 `/api/config` 下发给前端与桥接页；
桥接页只认**它自己 origin** 上的这份配置，绝不采信父页面自报的名单。

镜像**只支持两条线路**：协议按「一对线路互为镜像」设计（共用一条待同步队列、
靠对端回执出队），三条以上需要改造成每对端独立队列，因此保存时超过两条会被拒绝。

### 线路镜像的三条约束

| 约束 | 原因 |
|------|------|
| **两条线路用配置里的地址都能打开同一个站点** | 对端 origin 要与 `location.origin` 全等匹配。用未配置的地址访问（例如配了域名却用 IP 打开）时镜像静默不启用——按 F12 在控制台会看到一条说明。 |
| **不得给站点加 `X-Frame-Options` 或 CSP `frame-ancestors`** | 镜像依赖同门（或多线路间）iframe 嵌入对端页面。加上这两个头，镜像立刻失效（应用本身照常可用，只是退回手动导出/导入）。 |
| **镜像的新鲜度上限 = 双方最后一次都能连通的时刻** | 每条线路的副本只能由对方来更新。对方不可达时，自己的副本停在那一点；恢复连通后自动按条目合并回来，不会丢数据。这是「不借助任何第三方媒介」的必然代价。 |

另外：同一台设备上数据会**存两份**（每个 origin 各一份）；跨 origin 的 iframe
在 https 页面里加载 http 页面会被浏览器当作混合内容拦掉，所以两条线路要么都
https、要么都 http（内网 IP 测试的情形）。镜像不可用时会静默降级，应用行为退回
单线路；右栏底部给出提示，可用「导出 / 导入」手动搬运。

**两条线路要一起升级。** 历史条目里「重新生成」留下的版本数组（`versions`）只有
新版本的前端认识：旧版本页面收到带版本的条目时，会按老形状裁剪掉这个字段，再按
整条 LWW 覆盖回对端——那一版的旧结果就真丢了。导出的备份文件同理：新版本导出的
文件在旧版本页面上会因校验和不匹配被拒（提示「或用更新版本的应用导出过」，属预期，
不是文件损坏）。反方向没有这个问题：旧版本导出的文件、旧线路推来的数据，新版本
一律照常接收（新字段缺席时形状与从前逐字节一致）。

### 容量与运维

后台数据会随使用持续增长（`usage_logs` 每次 LLM 调用一行；标题失败重试时同一任务可能留下多行），部署一段时间后按下表检查。**默认配置下使用日志永久保留；`title_jobs` 的任务正文会在终态立即清空，状态元数据自动保留 7 天。**

| 项 | 说明与建议 |
|----|------------|
| `LOG_RETENTION_DAYS` | 默认 `0`（永久保留）。设为正数后每日自动清理超期日志（含原始数据）。**注意：免费模型的防滥用限额按 `usage_logs` 计数，保留期设得比限额窗口（最长 30 天）短会让计数偏小**，需要限额跨月生效时请保持 0 或至少 ≥ 30。 |
| 标题任务 | `title_jobs` 在任务终态立即清空 `input_text/output_text`；成功标题、失败状态等元数据保留 7 天后自动删除。运行中的任务会在服务重启后恢复。 |
| 日志记录的原始数据 | `log_payload` 开关打开后，`log_payloads` 每行最多存 3×6 万字符（约 180KB）。按每天千次调用估算，一天可增加上百 MB，长期开启请务必同时设置保留期。 |
| 数据库缩容 | 清理旧日志只删行，SQLite 文件不会自动缩小。需要回收磁盘时手工执行 `VACUUM`（会长时间独占数据库文件，请在低峰期、先停掉主站与管理后台两个进程）。 |
| 设备表 | `devices` 每个新浏览器指纹一行，**没有自动清理**（清日志不会删设备）。指纹由客户端上报、可被伪造，若发现异常增长，可在管理后台按最近活跃时间人工排查。 |
| 查询性能 | `usage_logs` 已按「身份（code_id / fingerprint / ip）+ 模型 + 时间」建复合索引，免费模型限额计数与设备列表排序不随日志量线性变慢；索引占用随行数增长，属正常开销。 |

---

## 10. 下一步 Prompt 补全清单

以下 **14 个工具** 的 Prompt 需要后续人工补全或打磨。其中 9 个为后端生成的简单初版，5 个为基于原始指令的部分覆盖版本。

此外 `prompts/` 下还有两类非工具 Prompt（不属于上面的补全清单）：

- `继续生成.md` — 所有工具共用的续写指令：生成中断后点「继续生成」时，它作为最后一条 user 消息发出（见 `docs/API_CONTRACT.md` 8.1）。改它即时影响全部工具的续写表现。
- `智能错题迁移错因分析More.md` — 智能错题迁移「More」再分析的专用指令。

### 9 个完全无原始 Prompt 的工具

| 工具 ID | 工具名称 | 当前 Prompt 状态 | 应实现的功能描述 | Prompt 文件路径 |
|---------|----------|----------------|------------------|----------------|
| 2 | 言说策略分析 | 简单初版 | 输入语篇，识别并分析“怎么说”的策略：修辞手法、语气变化、衔接与过渡、逻辑推进、情态/态度标记，输出分项策略清单及对阅读与写作教学的启示 | `prompts/言说策略分析.md` |
| 8 | 语法情境教学 | 简单初版 | 输入目标语法点 + 学段，输出情境化教学方案：真实语境例句、交际任务、典型错误预测、形成性练习与评价标准 | `prompts/语法情境教学.md` |
| 9 | 英文写作教学 | 简单初版 | 覆盖记叙/议论/概要等通用文体；输入文体 + 学段，输出结构支架、过程性范文、升格路径、常见问题清单 | `prompts/英文写作教学.md` |
| 10 | 学生作文批改 | 简单初版 | 输入学生作文，输出 5 维度评分、逐段点评、错误归类表、升格版 | `prompts/学生作文批改.md` |
| 12 | 试卷重点题讲评 | 简单初版 | 输入错题或重题，输出错因诊断、解题范式拆解、变式训练题、迁移应用题 | `prompts/试卷重点题讲评.md` |
| 13 | 试卷可视化全解 | 简单初版 | 输入整卷，输出课堂投影版结构化解析：题干与答案可分步显示、分题型分组、答案可隐藏 | `prompts/试卷可视化全解.md` |
| 21 | 辅助完形填空命题 | 简单初版 | 输入语篇（280-320 词），输出设空方案、4 选 1 选项、答案、解析、考点标签 | `prompts/辅助完形填空命题.md` |
| 22 | 试题解读分析 | 简单初版 | 输入单题或整卷，输出选材立意、能力考查维度、教学反拨启示、课标对标分析 | `prompts/试题解读分析.md` |
| 23 | 英语试题 Bug 侦察 | 简单初版 | 输入试题文本，输出 bug 清单：拼写/标点/中式英语/语法错误/逻辑漏洞/题干歧义/文化不当，逐条标注位置 + 改写建议 | `prompts/英语试题 Bug 侦察.md` |

### 5 个部分覆盖原始 Prompt 的工具

| 工具 ID | 工具名称 | 当前 Prompt 状态 | 应实现的功能描述 | Prompt 文件路径 |
|---------|----------|----------------|------------------|----------------|
| 4 | 课文语言点讲解 | 部分覆盖 | 基于语篇结构化知识讲解语言点（词汇、语法、句法、文化背景等） | `prompts/课文语言点讲解.md` |
| 6 | 词汇分类教学 | 部分覆盖 | 词汇分类 + 功能化教学材料 | `prompts/词汇分类教学.md` |
| 7 | 词块提取与讲解 | 部分覆盖 | 高价值词块提取 + 讲解 + 操练活动 | `prompts/词块提取与讲解.md` |
| 20 | 阅读理解设问 2 | 部分覆盖 | 阅读理解设问的另一变体，与“阅读理解设问 1”形成互补 | `prompts/阅读理解设问 2.md` |
| 25 | 自由对话 | 部分覆盖 | 通用 LLM 对话入口，提示词调试 | `prompts/自由对话.md` |
| 32 | 识别图片文字 | 已完成 | 拍照/上传试卷与手写作文转文字（印刷试卷、手写作文两份提示词，按所在工具自动选，只有本工具与自由对话给用户选）；图片可转正、裁剪后再识别 | `prompts/识别图片文字-印刷试卷.md`、`prompts/识别图片文字-手写作文.md` |
| 33 | 翻译 | 新增 | 原文译文对照翻译：语言对可选（默认 自动检测 → 简体中文），原文可粘贴、上传文档或拍照识别；改原文后重新翻译写回同一条历史记录 | `prompts/翻译.md`（续写：`prompts/翻译续写.md`） |

---

## 11. 注意事项

1. **安全**：生产环境务必修改 `JWT_SECRET`；管理后台 8001 仅内网访问，不映射到公网。
2. **密钥**：妥善保管 `backend/.env` 中的 API Key，不要提交到 Git。该文件会被构建进 Docker 镜像，请注意镜像的访问权限。
3. **使用码**：管理后台零登录、可直接生成使用码；首次启动若库中无码，会自动创建一把无限额度使用码并写入数据目录的 `bootstrap_code.txt`。
4. **Prompt**：当前 Prompt 文件为占位版本，后续需人工补全 14 个待补工具。想给某个工具单独定制续写行为时，在 `prompts/` 下加一份 `<工具名>续写.md` 即可覆盖共用的 `继续生成.md`（无需改代码，重启后生效——Prompt 只在进程启动时加载一次）。
5. **历史/收藏**：清理浏览器数据会丢失历史记录与收藏。

---

## 12. 变更记录

| 版本 | 日期 | 说明 |
|------|------|------|
| 1.0.0 | 2026-07-24 | 阶段一：后端 API + 25 工具 Prompt + demo 前端 |
| 1.1.0 | 2026-07-28 | 阶段一补充：使用码系统、管理后台、后端 SQLite、双端口部署 |
| 1.2.0 | 2026-08-07 | 新增独家功能“智能错题迁移”：错因确认、并行迁移卡片、批量操作与分阶段计费 |
| 1.2.1 | 2026-09-06 | 对齐上游新增 5 个教学工具：教考衔接文本解读、优质例句创研工坊、依托语境的词汇练习、隐性写作知识显性化、读后续写审题指导 |
| 1.9.0 | 2026-09-13 | 使用码不再区分类型：移除 code_type（数据库列在启动时自动迁移删除），管理后台零登录不需要码；初始使用码改为无限额度普通码并写入 bootstrap_code.txt |
| 1.10.0 | 2026-09-21 | 图片识别「识别图片文字」（工具 32）：拍试卷 / 手写作文转可编辑文字，可一次多张整卷识别；**转录类型由所在工具决定**（作文批改用「手写作文」规则，其余工具用「印刷试卷」规则，只有本工具与自由对话保留手动选择）；上传入口统一为「上传」按钮并新增来源弹窗（已有原稿 / 拍照；「识别图片文字」本身就是把照片转文字，直接进拍照那一步），支持系统相册多选、触摸设备用原生相机，以及**手机扫码拍照直传服务器**（手机只负责拍与传、图片只上传一次不落盘，筛选/排序/开始识别全在电脑上拍板；手机页沿用主站设计语言并自动跟随电脑端当前主题）；模型用途改为勾选式能力位（用户可用 / 用于 OCR / 用于 Chores，禁用为上一级开关），新增 OCR 模型与输出上限配置 |
| 1.11.0 | 2026-09-21 | 图片识别新增**查看即编辑**：点图放大变成编辑器，可 90° 转正、拖框裁剪，**退出裁剪模式立刻看到裁完的样子**（实时预览：只把框外的部分不显示，不动原图也不重新编码，与点「完成」烘焙出来的字节一致）；顶部居中一条深色玻璃工具条（宽屏带文字说明，窄屏只有图标）加右侧醒目的「完成」，电脑上滚轮缩放与 `[` `]` 快捷键、手机上双指缩放与加大的把手热区（悬停反馈只在有指针设备时给，触屏不会粘住）；旋转与裁剪在同一次画布运算里完成（只重新编码一次），点「完成」即落地并顺带把结果退回「开始识别」，第一次动图前留底、随时「还原原图」 |
| 1.12.0 | 2026-09-21 | 修复升级到 1.10.0 后旧部署暴露的三个问题：「禁用模型」开关改为「启用模型」（此前标签与正向的 `enabled` 字段方向相反，打开开关即启用却写着"禁用"）；旧单 URL 配置迁移改为直读环境配置与库中旧键（此前从配置白名单取两个早已被移出的旧键，恒为空，导致迁移出的 Provider 地址为空且不再修正），并会在启动时补齐已存在的空地址 Provider；静态资源改为「未版本化一律每次重验证」，避免升级后浏览器继续用旧脚本（管理端资源版本串同步提升） |
| 1.13.0 | 2026-09-23 | 新增「翻译」工具（工具 33，通用工具）：原文译文左右对照（窄屏上下分屏），两栏按滚动进度同步对齐（强制开启，两端位置对齐、回声不互相追），原文栏可折叠、折叠时译文居中；语言对可选（默认 自动检测 → 简体中文，原文选简体中文时译文自动换英语（美式））；原文支持粘贴、上传 Word / PDF / TXT 与拍照识别（印刷试卷 / 手写作文）；改原文后点「更新翻译」**写回同一条历史记录**而不新增，撞输出上限时可「继续翻译」接着译完；思考过程只在思考期间出现，正文一出即整条隐藏。同批修复：**本地静态资源不再带 `?v=` 版本串**（`no-cache` + `etag` 自动跟进，升级后不需要强刷，手机端尤其重要）；窄屏输入法弹起时译文栏收成一行把高度让给原文栏 |
