...
- **图片识别（识别图片文字）**：在「参考技能」里拍照或上传试卷、手写作文，转成可编辑文字。转录规则由所在工具决定，不需要用户选：作文批改收到的学生手写稿按涂改后的最终状态逐字转录（保留拼写与语法错误），其余工具一律只转录印刷原文（学生答案、勾画、批注自动忽略）；只有「识别图片文字」和「自由对话」两种素材都可能出现，才在界面上给选择。一次可拍多张（整卷多页），其他工具的「上传」入口也能直接走识别，识别结果落进输入框继续用。**点图放大即是编辑器**：可转正、可裁掉桌面与边缘，裁剪退出裁剪模式就见到结果（实时预览，不改动原图也不重新编码），电脑上拖框与滚轮缩放、手机上双指缩放拖把手，满意了点「完成」写入这张图片，第一次动图前留底、随时「还原原图」。
