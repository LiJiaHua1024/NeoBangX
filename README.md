...
- **图片识别（识别图片文字）**：在「参考技能」里拍照或上传试卷、手写作文，转成可编辑文字。转录规则由所在工具决定，不需要用户选：作文批改收到的学生手写稿按涂改后的最终状态逐字转录（保留拼写与语法错误），其余工具一律只转录印刷原文（学生答案、勾画、批注自动忽略）；只有「识别图片文字」和「自由对话」两种素材都可能出现，才在界面上给选择。一次可拍多张（整卷多页），其他工具的「上传」入口也能直接走识别，识别结果落进输入框继续用。
...
| 32 | 识别图片文字 | 已完成 | 拍照/上传试卷与手写作文转文字（印刷试卷、手写作文两份提示词，按所在工具自动选，只有本工具与自由对话给用户选） | `prompts/识别图片文字-印刷试卷.md`、`prompts/识别图片文字-手写作文.md` |
