...
- **双线路支持**：同一套应用可由多条线路（主线路 + 备份线路）提供服务，历史与收藏在两条线路间自动镜像；也可导出 / 导入 JSON 手动搬运。见 [9. 数据存储](#9-数据存储)。
- **多主题切换**：毛玻璃 / 科技感视觉风格。
...
| 能力 | 说明 |
|------|------|
| 导出 / 导入 | 右栏底部两个按钮，把历史与收藏导出成一份 JSON，可再次导入并与本机数据合并（按条目 `id` 取新）。用于备份、换设备、以及镜像失效时手动搬运。文件由用户自己保管，不经过后端。 |
| 线路镜像 | 多条线路（主线路 + 备份线路）互嵌一个隐藏的同站 iframe，把本机变更推进对方 origin 自己的 localStorage。**方向是「推」不是「拉」**：备份线路要顶的正是主线路挂掉的那一刻，那时主线路的页面加载不了、也就读不到它的存储，提前推进去故障时才有得用。无后端参与数据。 |
