...
- **31 个中学英语教学 AI 工具**：新增独家功能“智能错题迁移”，覆盖语篇分析、教学设计、命题辅助、作文批改、试卷讲评等。
- **使用码系统**：使用码即登录凭据，无需注册；无码不可用，按次计费，用完锁定。
- **管理后台**：内网访问，生成/管理使用码、查看使用日志、配置 LLM API。
- **流式生成**：SSE 打字机效果，生成过程可中止。
- **历史 / 收藏**：存于浏览器 localStorage（后端不存用户数据）。
- **双线路支持**：同一套应用可由多条线路（主线路 + 备份线路）提供服务，历史、收藏与主题 / 模型偏好都在两条线路间自动镜像；也可导出 / 导入 JSON 手动搬运。见 [9. 数据存储](#9-数据存储)。
- **多主题切换**：毛玻璃 / 科技感视觉风格。
...
> **升级提示**：v1.9.0 起使用码不再区分类型，首次启动会自动删除数据库中的 `code_type` 列（不可逆），升级前建议备份数据卷中的 `neobangx.db`。
...
所有使用码统一为普通使用码（`NBXU` 前缀），额度支持有限次数或无限（`-1`），不再区分管理员/用户类型。
...
- 每次工具生成扣减 1 次额度。
- 无限额度码（额度 `-1`）不扣减次数，但仍记录使用日志。
- 额度用完后直接锁定，无法继续使用。
- 管理后台可把某个使用码的已用次数重置为 0（剩余额度随之恢复，使用日志保留）。
- 管理后台可把使用码额度改为无限（已用清零），也可从无限改回有限（已用从 0 开始计数）。
...
- `PUT /api/admin/config` — 更新配置（含日志记录开关与保留天数、线路镜像开关与线路地址）
...
| 历史记录 | 浏览器 localStorage（列表索引 + 每条正文分开存，超出配额时自动淘汰最旧记录） |
| 收藏 | 浏览器 localStorage |
| 主题 / 选中模型 | 浏览器 localStorage（`nbx_theme` / `nbx_model` 为当前值，`nbx_prefs` 存镜像同步用的带时间戳副本） |

后端始终是**无状态代理**，不存储任何用户数据；历史与收藏只存在用户浏览器里。
上面这张表里唯一「多出来」的两行，是为了让同一套应用的两条线路（主线路与备份线路）
能共用同一份历史、收藏与偏好设置：

| 能力 | 说明 |
|------|------|
| 导出 / 导入 | 右栏底部两个按钮，把历史、收藏与主题 / 模型偏好导出成一份 JSON，可再次导入并与本机数据合并（条目按 `id` 取新，偏好按时间戳取新）。用于备份、换设备、以及镜像失效时手动搬运。文件由用户自己保管，不经过后端。 |
| 线路镜像 | 多条线路（主线路 + 备份线路）互嵌一个隐藏的同站 iframe，把本机变更推进对方 origin 自己的 localStorage。**方向是「推」不是「拉」**：备份线路要顶的正是主线路挂掉的那一刻，那时主线路的页面加载不了、也就读不到它的存储，提前推进去故障时才有得用。无后端参与数据。历史、收藏、主题与选中模型一并镜像（同一套 LWW 判据），对端有、本线路没有的主题 / 模型不套用。 |

### 线路镜像的配置

默认**关闭**（单线路部署开启它没有任何作用）。开关与线路地址在
**管理后台 →「线路镜像」**里设置，也可用环境变量 `MIRROR_ENABLED` / `MIRROR_ORIGINS`
（管理后台的保存会覆盖环境变量的值）：

```ini
MIRROR_ENABLED=true
# 逗号分隔，必须是纯 origin（含协议与端口，不带路径）
MIRROR_ORIGINS=https://www.example.com,https://cf.example.com
# 内网 / IP 测试同样可用
# MIRROR_ORIGINS=http://192.168.1.10:8000,http://192.168.1.10:8001
```

地址一律填**完整 origin**：前端用它拼 `{origin}/static/bridge.html`，并与
`location.origin` / `event.origin` 做全等比较，所以带路径、漏端口都会让比对失败。
两条线路是对等的（不分主备，顺序随意）。代码里没有任何硬编码域名——开源部署
各有各的域名，内网测试还可能是 IP + 端口。配置从 `/api/config` 下发给前端与桥接页；
桥接页只认**它自己 origin** 上的这份配置，绝不采信父页面自报的名单。

镜像**只支持两条线路**：协议按「一对线路互为镜像」设计（共用一条待同步队列、
靠对端回执出队），三条以上需要改造成每对端独立队列，因此保存时超过两条会被拒绝。

### 线路镜像的三条约束

| 约束 | 原因 |
|------|------|
| **两条线路用配置里的地址都能打开同一个站点** | 对端 origin 要与 `location.origin` 全等匹配。用未配置的地址访问（例如配了域名却用 IP 打开）时镜像静默不启用——按 F12 在控制台会看到一条说明。 |
| **不得给站点加 `X-Frame-Options` 或 CSP `frame-ancestors`** | 镜像依赖同门（或多线路间）iframe 嵌入对端页面。加上这两个头，镜像立刻失效（应用本身照常可用，只是退回手动导出/导入）。 |
| **镜像的新鲜度上限 = 双方最后一次都能连通的时刻** | 每条线路的副本只能由对方来更新。对方不可达时，自己的副本停在那一点；恢复连通后自动按条目合并回来，不会丢数据。这是「不借助任何第三方媒介」的必然代价。 |

另外：同一台设备上数据会**存两份**（每个 origin 各一份）；跨 origin 的 iframe
在 https 页面里加载 http 页面会被浏览器当作混合内容拦掉，所以两条线路要么都
https、要么都 http（内网 IP 测试的情形）。镜像不可用时会静默降级，应用行为退回
单线路；右栏底部给出提示，可用「导出 / 导入」手动搬运。

### 容量与运维

后台数据会随使用持续增长（`usage_logs` 每次 LLM 调用一行，一次用户操作通常两行：生成 + 标题生成），部署一段时间后按下表检查。**默认配置下日志永久保留，磁盘只增不减。**

| 项 | 说明与建议 |
|----|------------|
| `LOG_RETENTION_DAYS` | 默认 `0`（永久保留）。设为正数后每日自动清理超期日志（含原始数据）。**注意：免费模型的防滥用限额按 `usage_logs` 计数，保留期设得比限额窗口（最长 30 天）短会让计数偏小**，需要限额跨月生效时请保持 0 或至少 ≥ 30。 |
| 日志记录的原始数据 | `log_payload` 开关打开后，`log_payloads` 每行最多存 3×6 万字符（约 180KB）。按每天千次调用估算，一天可增加上百 MB，长期开启请务必同时设置保留期。 |
| 数据库缩容 | 清理旧日志只删行，SQLite 文件不会自动缩小。需要回收磁盘时手工执行 `VACUUM`（会长时间独占数据库文件，请在低峰期、先停掉主站与管理后台两个进程）。 |
| 设备表 | `devices` 每个新浏览器指纹一行，**没有自动清理**（清日志不会删设备）。指纹由客户端上报、可被伪造，若发现异常增长，可在管理后台按最近活跃时间人工排查。 |
| 查询性能 | `usage_logs` 已按「身份（code_id / fingerprint / ip）+ 模型 + 时间」建复合索引，免费模型限额计数与设备列表排序不随日志量线性变慢；索引占用随行数增长，属正常开销。 |
...
| 1.2.1 | 2026-09-06 | 对齐上游新增 5 个教学工具：教考衔接文本解读、优质例句创研工坊、依托语境的词汇练习、隐性写作知识显性化、读后续写审题指导 |
| 1.9.0 | 2026-09-13 | 使用码不再区分类型：移除 code_type（数据库列在启动时自动迁移删除），管理后台零登录不需要码；初始使用码改为无限额度普通码并写入 bootstrap_code.txt |
