...
- **32 个中学英语教学 AI 工具**：新增独家功能“智能错题迁移”，覆盖语篇分析、教学设计、命题辅助、作文批改、试卷讲评等。
- **图片识别（识别图片文字）**：在「通用工具」里拍照或上传试卷、手写作文，转成可编辑文字。转录规则由所在工具决定，不需要用户选：作文批改收到的学生手写稿按涂改后的最终状态逐字转录（保留拼写与语法错误），其余工具一律只转录印刷原文（学生答案、勾画、批注自动忽略）；只有「识别图片文字」和「自由对话」两种素材都可能出现，才在界面上给选择。一次可拍多张（整卷多页），其他工具的「上传」入口也能直接走识别，识别结果落进输入框继续用。**点图放大即是编辑器**：可转正、可裁掉桌面与边缘，裁剪退出裁剪模式就见到结果（实时预览，不改动原图也不重新编码），电脑上拖框与滚轮缩放、手机上双指缩放拖把手，满意了点「完成」写入这张图片，第一次动图前留底、随时「还原原图」。
...
├── prompts/                     # 32 个工具 Prompt 文件 + 共用的续写指令
├── Dockerfile
├── docker-compose.yml
...
**改动静态文件后必须提升 `?v=`**：`frontend/` 与 `admin-frontend/` 无构建步骤、文件名不带
内容哈希，缓存失效完全靠 `index.html` 里手写的版本查询串；带 `?v=` 的资源按 immutable 长缓存，
不带 `?v=` 的按每次重验证处理。只改 JS/CSS 而不动版本串，用户浏览器会继续跑旧代码，
升级后出现「新页面 + 旧脚本」的错配（管理端曾因此让三个用途勾选框全部显示为未勾选）。
